...
{	
	ros::init(argc, argv, "sp1_node");
	ros::NodeHandle nh;
	swiftpro::SwiftproState swiftpro_state;

	ros::Subscriber sub1 = nh.subscribe("sp1_position_write_topic", 1, position_write_callback);
	ros::Subscriber sub2 = nh.subscribe("sp1_status_topic", 1, swiftpro_status_callback);
	ros::Subscriber sub3 = nh.subscribe("sp1_angle4th_topic", 1, angle4th_callback);
	ros::Subscriber sub4 = nh.subscribe("sp1_gripper_topic", 1, gripper_callback);
	ros::Subscriber sub5 = nh.subscribe("sp1_pump_topic", 1, pump_callback);
	ros::Publisher 	 pub = nh.advertise<swiftpro::SwiftproState>("sp1_state_topic", 1);
