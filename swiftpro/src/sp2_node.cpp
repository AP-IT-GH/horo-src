...
{	
	ros::init(argc, argv, "sp2_node");
	ros::NodeHandle nh;
	swiftpro::SwiftproState swiftpro_state;

	ros::Subscriber sub1 = nh.subscribe("sp2_position_write_topic", 1, position_write_callback);
	ros::Subscriber sub2 = nh.subscribe("sp2_status_topic", 1, swiftpro_status_callback);
	ros::Subscriber sub3 = nh.subscribe("sp2_angle4th_topic", 1, angle4th_callback);
	ros::Subscriber sub4 = nh.subscribe("sp2_gripper_topic", 1, gripper_callback);
	ros::Subscriber sub5 = nh.subscribe("sp2_pump_topic", 1, pump_callback);
	ros::Publisher 	 pub = nh.advertise<swiftpro::SwiftproState>("sp2_state_topic", 1);
	ros::Rate loop_rate(20);

	try
	{
		_serial.setPort("/dev/ttyACM1");
